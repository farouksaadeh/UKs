...
-- DESC = Heisst absteigend sortieren
-- ASC = Heisst aufsteigend sortieren
-- Aufgabe 4.1: Einfache Datenabfrage
SELECT * FROM product; 
SELECT * FROM category;
SELECT firstname, lastname, email FROM customer;
SELECT ordered_at ORDER BY order_entry DESC;
SELECT price ORDER BY product DESC;
SELECT name, price ORDER BY product DESC LIMIT 3;
SELECT name, price ORDER BY product ASC LIMIT 3;

--Aufgabe 4.2: Funktionen anwenden
--SQRT = Wurzel
--AS = Name für die Ausgabe der Zahl
--SQRT(HierMussDerSpaltenNameHinein)
SELECT SQRT(price) AS PRICE_SQRT FROM product; --Quadratwurzel

SELECT MONTHNAME(ordered_at) AS MonthName FROM order_entry; 

SELECT FirstName, LENGTH(FirstName) AS AnzahlBuchstaben FROM customer;
...
SELECT ROUND(price * 0.08, 2) AS mwst
FROM product;

SELECT COUNT(*) AS anzahl_datensätze
FROM product;

SELECT 
    MIN(price) AS mindestpreis, --MIN = kleinster Wert in der Tabelle product
    MAX(price) AS höchstpreis, --MAX = Grösster Wert in der Tabelle product
    AVG(price) AS durchschnittspreis --AVG = Durchschnittswert in der Tabelle product
FROM product;

SELECT 
    MIN(price) AS mindestpreis,
    MAX(price) AS höchstpreis, 
...
--Block / Aufgaben 5 in Dokumentation
